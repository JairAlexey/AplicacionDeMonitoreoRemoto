...
const DURATION_SEC = Number.parseInt(process.env.DURATION_SEC || "10", 10);
...
const main = async () => {
  ...
  const users = Math.min(CONCURRENCY, selectedKeys.length);
  const keys = selectedKeys.slice(0, users);
